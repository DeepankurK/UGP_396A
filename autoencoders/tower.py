...
.links as L
from  import cuda, Variable
from .initializers import Normal

class Encoder_text_tower(.Chain):
    """
    ...
    def __call__(self, x, objects_one_hot, descs_one_hot, train=True):
        with .using_config('train', train), .using_config('enable_backprop', train):
...
class Discriminator_texual(.Chain):
    """
    ...
    the end. The first 6 convoltion layers are shared and there is separated Fully-connected layers
