...
from  import cuda, Function, gradient_check, report, training, utils, Variable
from  import datasets, iterators, optimizers, serializers
...
class ModelController:
    ...
    def handle_gpu_batch(self, 
        ...
        # The features extracted by the Encoder is passed to the robot controller
        ...
        if self.train_autoencoder:
            ...
            self.gen_models[k].cleargrads()
            self.mdn_models[k].cleargrads()
            loss_net = loss_enc + loss_gen + loss_lstm
            loss_net.backward()
    ...
    def save_models(self):
        xp = cuda.cupy
        test_cost, test_lstm_loss = self.test_testset()
        if test_lstm_loss < self.last_best_result:
    ...
    def test_testset(self):
        ...
        with .using_config('train', False), .using_config('enable_backprop', False):
            ...
            for i in range(num_batches):
                ...
                test_lstm_loss += float(mdn_loss.data)
                test_rec_loss += float(batch_rec_loss.data)

        test_lstm_loss = test_lstm_loss / num_batches
        test_rec_loss = test_rec_loss / num_batches
        print '\nrecent test cost: ' + str(test_rec_loss) + ' recent lstm cost: ' + str(test_lstm_loss)
        return test_rec_loss, test_lstm_loss
    ...
    def load_model(self):
        try:
            ...
            serializers.load_hdf5(file_path + 'rnn_mdn_adverserial_e2e.model', self.mdn_model)
            serializers.load_hdf5(file_path + 'rnn_mdn_adverserial_e2e.state', self.optimizer_mdn)
